...
const xLabels = ["Year", "Year"]; // Modify as needed
const yLabels = ["Price (INR)", "Value (INR)"]; // Modify as needed

// X-axis labels and indices for each graph
const years0 = [2019, 2020, 2021, 2022, 2023, 2024];
const xAxisLabels0 = years0.map(String);
const xAxisIndices0 = [0, 12, 24, 36, 48, 60];

const years1 = [];
for (let y = 2012; y <= 2024; y++) {
  years1.push(y);
}
const xAxisLabels1 = years1.map(String);
const xAxisIndices1 = [];
for (let i = 0; i < years1.length; i++) {
  xAxisIndices1.push(i * 12);
}
...
function draw(price, index, xLabel, yLabel, xAxisLabels, xAxisIndices) {
  ...
  for (let i = 0; i <= 4; i++) {
    ...
    const yLabelElem = paper.text(-40, chartH - i * (chartH / 4), yLabelText);
    yLabelElem.attr({
      'fill': 'white',
      'font-size': '16px',
      'text-anchor': 'middle',
  ...
  const yLabelTextElem = paper.text(-80, chartH / 2.2, yLabel);
  yLabelTextElem.attr({
    'fill': 'white',
    'font-size': '16px',
    'text-anchor': 'middle',
  });
  yLabelTextElem.transform(`r-90,-80,${chartH / 2}`);

  // Add x-axis label
  const xLabelText = paper.text(chartW / 2, chartH + 50, xLabel);
  xLabelText.attr({
    'fill': 'white',
    'font-size': '16px',
    'text-anchor': 'middle',
  ...
  xAxisLabels.forEach((label, idx) => {
    const dataIndex = xAxisIndices[idx];
    const xPos = step(dataIndex, chartW);

    // Draw tick mark
    const tick = paper.path(`M${xPos},${chartH}L${xPos},${chartH + 5}`);
    tick.attr({
      'stroke': 'white',
      'stroke-width': 1,
    });
    ...
    // Draw label
    const xLabelElem = paper.text(xPos, chartH + 20, label);
    xLabelElem.attr({
      'fill': 'white',
      'font-size': '12px',
      'text-anchor': 'middle',
  ...
  const handleIntersection = (entries, observer) => {
    entries.forEach((entry) => {
      ...
      if (entry.isIntersecting) {
        ...
        // Determine which price array and x-axis labels to use based on the index
        let priceArray, xLabelText, yLabelText, xAxisLabels, xAxisIndices;

        if (index === 0) {
          priceArray = price0;
          xLabelText = xLabels[0];
          yLabelText = yLabels[0];
          xAxisLabels = xAxisLabels0;
          xAxisIndices = xAxisIndices0;
        } else {
          priceArray = price1;
          xLabelText = xLabels[1];
          yLabelText = yLabels[1];
          xAxisLabels = xAxisLabels1;
          xAxisIndices = xAxisIndices1;
        }

        // Draw and animate the graph
        draw(priceArray, index, xLabelText, yLabelText, xAxisLabels, xAxisIndices);
